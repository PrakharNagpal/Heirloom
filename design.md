...
**Colors**
```css
--rice:    #FBF7EE;  /* app background, cards */
--lacquer: #163B3D;  /* primary text, dark screens (Record), nav icon bg */
--kueh-rose: #D96A8A; /* primary accent — CTAs, record button, active states */
--jade-tint: #eef3ed; /* soft green surface (avatar bg, storybook format card) */
--rose-tint: #fdeef1; /* soft rose surface (avatar bg, cookalong format card) */
--sand-tint: #f2ede0; /* soft neutral surface (phrase coach card, tip callouts, active language pill) */
--pandan:  #7FA650;  /* step-number badges, success/complete */
--gold-leaf: #C9A227; /* gap-prompt / "ask her" callouts — dashed border */
--border:  #ece3d2;  /* card borders */
--muted:   #7c8a84;  /* secondary text */
--muted-2: #a9b6b1;  /* tertiary text, timestamps */
--page-bg: #EDE6D6;  /* surrounding canvas behind the phone, not part of the app itself */
```

**Type**
- Display: `Fraunces` (weights 500/600, italic 500 for quoted speech) — screen titles, section headers, storybook captions, phrase-coach headwords.
- Body: `Noto Sans` (400/500/600/700) + `Noto Sans SC` for Chinese + `Noto Sans Tamil` for Tamil — load all three, switch by active language.
...
- Home/Stories tabs: plain icon + 12px label, active color `--kueh-rose`, inactive `--muted-2`.
- **Record tab is visually larger and raised**: a 62px circular rose button with white 4px border and drop shadow, sitting ~26px above the bar's top edge (`margin-top: -26px` on the tab), label bold beneath it. This is the primary action — always reachable from anywhere in the app.
- Bar background: `rgba(251,247,238,0.96)` with a 1px top border in `--border`, blurred/elevated over content.

## Screens

### 1. Home
- Header row: "Good morning" (15px muted) + "Your family's stories" (Fraunces 600, 26px) on the left; a 52px round avatar circle on the right (soft jade tint bg, emoji placeholder — swap for the user's real photo).
...
- Primary CTA button: full-width, rose background, rounded 18px. Left: 44px circle icon (🎙️) on rice background. Right: "Record a new memory" (white, bold 18px) / "Just press play and let her talk" (14px, light rose).
- "Saved memories" section header (Fraunces 600, 19px).
...
- The question being asked, in quotes, Fraunces italic 22px, centered.
- Helper line: "Ask her one question. Let her talk." (15px, muted teal `#9fc2bd`).
- Live waveform: row of vertical bars (rose, varying height) animating with audio level.
- Big record/stop control: 84px circle, rose fill, 6px rice border.
- Timer/status line beneath: "Tap to finish · 1:42".

### 3. Memory / transcript spine
- Back link "‹ All memories" (muted, 15px).
- Header row: 56px round avatar + memory title (Fraunces 600, 21px) + "Recorded today · 1:42" (muted).
- **Language switcher**: pill row, one pill per language (English / 中文 / Melayu / தமிழ்). Active pill: rose bg, white text. Inactive: sand-tint bg, muted text. Switching re-renders every segment's text in that language's font, instantly, without touching her audio.
- **Transcript spine** (signature element): a white card containing a list of segment rows. Each row: 38px dark circular ▶ button (tap plays that exact audio slice), the segment text (sized 16.5px, in the active language's font), and a mono timestamp on the right. The currently-active/tapped segment gets a sand-tint highlight background.
...
- Back link to the memory, then title "Ah Ma's [dish], step by step" (Fraunces 600, 24px) + meta line "Cook-along · serves N · tap 🔊 to hear her".
- One card per step: green (`--pandan`) numbered circle, instruction text (17px semibold), a small dark 🔊 button on the right that plays her original audio for that exact step (via its `segmentIndex`).
  - **Tip callout** (when present): sand-tint background, italic Fraunces, prefixed 💡 — her own aside, e.g. "my mother always added more sugar."
...
- Same header pattern. One card per phrase: original dialect word/phrase (Fraunces 600, 19px) + 🔊 button; romanisation in mono below; meaning (15.5px); "When to use it: …" line (muted).

### 6. Lesson player — Storybook
- Same header pattern, meta line "6 panels · illustrated · for the youngest grandchild".
- One card per panel: full-width 3:2 illustration (consistent illustrated style across all 6 — soft gouache children's-book look, warm palette, no text baked into the image), a numbered rose badge, the caption (Fraunces 17px), and a 🔊 button that plays her audio for that panel's segment.
